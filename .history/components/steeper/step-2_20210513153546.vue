...
export default {
  ...
  created () {
    (this.$nuxt || EventBus || this.$EventBus).$once('show-alert', this.showAlert)
  },
  methods: {
    next () {
      this.$emit('next')
    ...
    showAlert (msg) {
      this.getData.name = msg.name
      this.getData.area = msg.area
      this.getData.city = msg.city
      this.getData.address = msg.address
      this.getData.tel = msg.tel
      this.getData.zip = msg.zip
    }
  }
}
</script>
