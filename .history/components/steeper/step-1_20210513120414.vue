  ...
  <div>
    <div class="info">
      ...
      <div class="cus-input">
        ...
        <el-select v-model="area" class="area-input" placeholder="區域">
          <el-option value="">
            請選擇
          </el-option>
          <el-option v-for="(item, index) in districts" :key="index" :value="item">
            {{ item }}
          </el-option>
        </el-select>
      </div>
...
export default {
  ...
  computed: {
    ...
    districts () {
      let districts = []
      if (this.city !== '') {
        const getDistricts = postData.find((item) => {
          return item.name === this.city
        })
        districts = getDistricts.map((item) => {
